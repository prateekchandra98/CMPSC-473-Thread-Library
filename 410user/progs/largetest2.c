...
void barrier()
{
    mutex_lock(&bar_mutex);
    if (error) {
        mutex_unlock(&bar_mutex);
        return;
    }
    bar_count++;
    if (bar_count == bar_limit) {
        cond_broadcast(&bar_cond);
        mutex_unlock(&bar_mutex);
        return;
    }
    if (bar_count < bar_limit) {
        cond_wait(&bar_cond, &bar_mutex);
    }
    mutex_unlock(&bar_mutex);
}
...
int main() {
    ...
    mutex_destroy(&bar_mutex);
    cond_destroy(&bar_cond);
