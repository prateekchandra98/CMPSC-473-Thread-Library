...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provision :shell, :inline => "dpkg --add-architecture i386"
  config.vm.provision :shell, :inline => "apt-get update"
  config.vm.provision :shell, :inline => "apt-get install -y build-essential gcc-multilib emacs htop gdb flex bison libncurses5-dev libx11-dev libgtk2.0-dev mtools libtinfo5:i386 libncurses5:i386"
